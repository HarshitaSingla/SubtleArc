...
// // Add Leaflet.Editable typings if missing
...
//     this.onMapReady(mapInstance);
//   }

//   onMapReady(mapInstance: L.Map) {
//     this.map = mapInstance;
//     (this.map.options as any).editable = true;

//     if (!(this.map as any).editTools && (L as any).Editable) {
//       this.map.editTools = new (L as any).Editable(this.map);
//       console.log('✅ editTools initialized');
//     } else {
//       console.warn('⚠️ Leaflet.Editable not found or already initialized.');
//     }
//   }
...
//           console.error('❌ Map or editTools is not ready.');
//           return;
//         }
...
//         const editTools = this.map.editTools;
//         const allLatLngs: L.LatLng[] = [];
//         const zoom = this.map.getZoom();
//         const steps = Math.max(4, Math.floor(zoom * 1.5));
...
//         this.map!.eachLayer((layer) => {
//           if ((layer as any)._path || layer instanceof L.CircleMarker) {
//             if (!(layer as any)._url) {
//               this.map!.removeLayer(layer);
//             }
//           }
...
//     // Initialize leaflet-editable if available
